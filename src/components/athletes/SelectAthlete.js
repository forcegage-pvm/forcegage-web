...
export default class SelectAthlete extends React.Component {
    constructor(props) {
        ...
        client.query({
            ...
            query: gql`
            {people{id, name, surname, age, gender, bodyweight}}
            `,
        })
            .then(data => {
                var rowdata = []
    ...
    render() {
        ...
        return (
            <React.Fragment>
            <Row><
                Table style={{ width: "600px" }} rowSelection={this.rowSelectionAthlete} columns={AthleteColumns} dataSource={athleteData} size="small" pagination={true} /></Row>
            <Row style={{ display: "flex" }}>
                <Button type="primary" style={{ marginLeft: "460px" }} onClick={()=> {this.props.onSelected(SelectedAthleteIds)}}>
                    Select sessions
                    <Icon type="right" />
                </Button>
            </Row>
